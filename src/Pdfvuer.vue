...
export function createLoadingTask(src, options) {
  ...
  var loadingTask = pdfjs.getDocument(source).promise;
...
export default {
  ...
  mounted: function () {
    ...
    if (self.annotation) {
      annotationLayer = new pdfjsViewer.DefaultAnnotationLayerFactory();
    }
    if (self.text) {
      textLayer = new pdfjsViewer.DefaultTextLayerFactory();
    }
    ...
    self.internalSrc
      .then(function (pdfDocument) {
        ...
        self.pdfViewer = new pdfjsViewer.PDFPageView({
          container: container,
          id: self.page,
          scale: 1,
